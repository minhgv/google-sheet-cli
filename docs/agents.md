...
- **Offline by default where possible.** `workbook:inspect|read|find|write`, `report:run` with local input/output, and `grid:insert|delete`, `format:merge`, `data:clear` with `--workbook` need no credentials and make zero network calls. An agent can manipulate local Excel files and generate reports in a sandbox with no secrets at all.
...
- **Local workbooks are structurally mutable.** `grid:insert|delete --workbook <f.xlsx>` shift rows/columns and manage merges explicitly (a merge intersecting the splice boundary refuses unless `--force`); `format:merge --workbook` merges/unmerges; `data:clear --workbook` clears a bounded range; `workbook:write --cells '{...}'` writes only the listed addresses. Mutations land on `--output` or `--inPlace` (with `.bak` backup), never implicitly on the source. Formula references are never rewritten — receipts report `formulasAtRisk`; charts/pivots/macros cannot be preserved and saving a workbook that has them needs `--discardUnsupported`.
